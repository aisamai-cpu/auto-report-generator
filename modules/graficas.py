...
def grafica_ventas_por_mes(ventas_mes, ruta_salida):
    ...
    bars = ax.bar(ventas_mes.index.astype(str), ventas_mes.values, color='#2E86AB', edgecolor='white', linewidth=0.8)

    for bar, valor in zip(bars, ventas_mes.values):
        ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + valor * 0.02,
                f'${valor:,.0f}', ha='center', va='bottom', fontsize=9, fontweight='bold')

    ax.set_title('Ventas Totales por Periodo', fontsize=14, fontweight='bold', pad=15)
...
def grafica_ventas_por_producto(ventas_producto, ruta_salida):
    ...
    ax.pie(ventas_producto.values,
           labels=ventas_producto.index,
           autopct='%1.1f%%',
           colors=colores[:len(ventas_producto)],
           startangle=140,
           wedgeprops={'edgecolor': 'white', 'linewidth': 2})

    ax.set_title('Distribucion de Ventas por Producto', fontsize=13, fontweight='bold', pad=15)
